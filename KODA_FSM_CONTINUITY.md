...
- `npm run check:fsm-pr-readiness` at the repo root is the merge/completion receipt. It checks source cleanliness, PR #20 draft/merge/review/check state, backend health, admin deployment shell reachability, storefront inventory bridge health, and the known admin UI/external publish approval gates without printing secrets. Use it on each wake before saying the work is merely blocked.
