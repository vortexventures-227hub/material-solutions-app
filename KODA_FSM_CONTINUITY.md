...
- `npm run smoke:fsm-live` at the repo root is the preferred one-command live gate. It checks backend `/health`, runs the admin deployment smoke, pulls production storefront env into a temp file, runs the full 11-channel Publish Button dry-run smoke, and asserts Facebook Marketplace, MachineryTrader, eBay, and Google Business Profile-specific guarded draft fields.
...
- Production backend deployment `53fbc80e-8766-4bb3-b0ac-7729f9f785be` is live on Railway and includes the Publish Button `dryRun`/`testMode` guard, Craigslist local-draft guardrails, Facebook Marketplace-specific guarded draft fields, MachineryTrader vendor credential readiness fields, eBay OAuth/business-policy readiness fields, Google Business Profile permission readiness fields, and expanded guarded manual draft channels.
