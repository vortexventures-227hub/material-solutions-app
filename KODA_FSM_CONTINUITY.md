...
- `npm run smoke:fsm-live` at the repo root is the preferred one-command live gate. It checks backend `/health`, runs the admin deployment smoke, pulls production storefront env into a temp file, runs the full 11-channel Publish Button dry-run smoke, and asserts Facebook Marketplace, MachineryTrader, EquipFinder, MachineryATS, eBay, LinkedIn, Forkliftaction Forum, and Google Business Profile-specific guarded draft fields.
...
- Production backend deployment `7ac74783-3c36-45e7-9fb6-c5167254bf4a` is live on Railway and includes the Publish Button `dryRun`/`testMode` guard, Craigslist local-draft guardrails, Facebook Marketplace-specific guarded draft fields, MachineryTrader vendor credential readiness fields, EquipFinder vendor/site readiness fields, MachineryATS vendor/DNS readiness fields, eBay OAuth/business-policy readiness fields, LinkedIn Company Page readiness fields, Forkliftaction Forum account/rules readiness fields, Google Business Profile permission readiness fields, and expanded guarded manual draft channels.
