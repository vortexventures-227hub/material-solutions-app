...
- `npm --silent run check:fsm-pr-readiness:json` emits the same readiness receipt as structured JSON so heartbeat wakes can parse exact blockers/check states and decide whether to keep working, wait for CI, or notify Chris.
