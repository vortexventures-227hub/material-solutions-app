...
- `.github/workflows/fsm-pr-checks.yml` gives PR #20 GitHub status checks for FSM script checks, backend tests, admin frontend build, and storefront build. Watch them after each push because the readiness receipt now blocks on missing, pending, failed, cancelled, skipped, or unknown checks.
- Required named checks for readiness are `FSM script checks`, `Backend tests`, `Admin frontend build`, and `Storefront build`; if one disappears from PR metadata, readiness is blocked even if the remaining checks are green.
...
- `FSM_PLATFORM_CONNECTION_FEASIBILITY.md` is the current 47-platform scope map. It clarifies that all 47 registry entries are connectable as Forklift Sales Machine destinations, but they are not all safe fully automatic public posting targets. Use its `automatic`, `api_or_feed`, `partner_or_portal`, and `guarded_manual` modes when choosing the next external channel.
