...
- `.github/workflows/fsm-pr-checks.yml` gives PR #20 GitHub status checks for backend tests, admin frontend build, and storefront build. Watch these after each push because the readiness receipt will surface their state through `gh pr view`.
