...
- PR readiness receipt: `npm run check:fsm-pr-readiness` at repo root checks source cleanliness, PR #20 draft/merge/review/check state, backend health, admin deployment shell reachability, storefront inventory bridge health, and the remaining admin UI/external publish approval gates without printing secrets.
