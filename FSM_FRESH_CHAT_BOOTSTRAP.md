...
- Backend tests: `npm test -- --runInBand` in `backend` passed 42/42.
...
- Aggregate live smoke: `npm run smoke:fsm-live` at repo root passed backend health, admin deploy bundle, 11-channel Publish Button dry-run checks, Facebook Marketplace-specific draft assertions, MachineryTrader vendor credential assertions, eBay OAuth/business-policy readiness assertions, and Google Business Profile permission readiness assertions.
