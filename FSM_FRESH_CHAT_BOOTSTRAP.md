...
- Machine-readable readiness receipt: `npm --silent run check:fsm-pr-readiness:json` emits the same gate state as JSON, including `nextAction`, classified blocker groups, and structured `gateActions`, for heartbeats and future automation.
